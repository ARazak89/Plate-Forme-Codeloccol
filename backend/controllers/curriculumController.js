...
export async function getCurriculumById(req, res) {
  try {
    ...
    if (!curriculum)
      return res
        .status(404)
        .json({ error: 'Parcours de formation non trouvé.' });
...
export async function updateCurriculum(req, res) {
  try {
    ...
    const curriculum = await Curriculum.findByIdAndUpdate(id, updatedData, {
      new: true,
      runValidators: true,
    });
    ...
    res.status(200).json({
      message: 'Parcours de formation mis à jour avec succès.',
      curriculum,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
export async function deleteCurriculum(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: 'Parcours de formation supprimé avec succès.' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
}
...
export async function assignStudentToCurriculum(req, res) {
  try {
    ...
    if (curriculum.students.includes(studentId)) {
      return res
        .status(400)
        .json({ message: 'Cet étudiant est déjà assigné à ce parcours.' });
    }
    ...
    res.status(200).json({
      message: 'Étudiant assigné au parcours avec succès.',
      curriculum,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
