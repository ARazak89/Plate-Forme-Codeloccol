...
export async function register(req, res) {
  try {
    ...
    const user = await User.create({ name, email, password: hash, role: role || 'apprenant' });
...
export async function login(req, res) {
  try {
    ...
    res.json({ token, user: { id: user._id, name: user.name, role: user.role, daysRemaining: user.daysRemaining, level: user.level } });
  } catch (e) {
    console.error(`Erreur lors de la connexion pour l'email: ${req.body.email || 'N/A'}: ${e.message}`);
    res.status(500).json({ error: e.message });
  }
}
...
export async function forgotPassword(req, res) {
  try {
    ...
    if (!user) {
      return res.status(404).json({ message: 'Aucun utilisateur avec cet email trouvé.' });
    }
