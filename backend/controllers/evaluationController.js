...
export async function getPendingEvaluationsAsEvaluator(req, res) {
  try {
    ...
    const evaluations = await Evaluation.find({ evaluator: evaluatorId, status: 'pending' })
...
export async function getAllPendingEvaluationsForStaff(req, res) {
  try {
    ...
    const projects = await Project.find({ status: { $in: ['pending', 'awaiting_staff_review'] } });
...
export async function submitEvaluation(req, res) {
  try {
    ...
    if (!evaluation.evaluator.equals(evaluatorId)) {
      return res.status(403).json({ error: 'Non autorisé à soumettre cette évaluation.' });
    }

    if (evaluation.status !== 'pending') {
      return res.status(400).json({ error: 'Cette évaluation a déjà été soumise ou n\'est plus en attente.' });
    }
    ...
    if (status === 'accepted') {
      ...
      if (!allFeedbackProvided) {
        return res.status(400).json({ error: 'Tous les champs de feedback sont obligatoires pour accepter le projet.' });
      }
    }
    ...
    const projectEvaluations = await Evaluation.find({ project: evaluation.project });

    // Vérifier si toutes les évaluations sont complétées (non en statut 'pending')
    const allPeerEvaluationsCompleted = projectEvaluations.every(evalItem => evalItem.status !== 'pending');

    const project = await Project.findById(evaluation.project);
    if (!project) {
      return res.status(404).json({ error: 'Projet non trouvé lors de la mise à jour du statut.' });
    }

    if (allPeerEvaluationsCompleted) {
      // Si au moins une évaluation est 'rejected', le projet est 'rejected'
      const anyRejected = projectEvaluations.some(evalItem => evalItem.status === 'rejected');
    ...
    res.status(200).json({ message: 'Évaluation soumise avec succès.', evaluation });
