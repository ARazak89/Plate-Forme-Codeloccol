...
import { createProject, listMyProjects, approveProject, rejectProject, submitPeerEvaluation, getGithubRepoTitle, assignProjectToStudent, getProjectDetails, submitProjectSolution, submitFinalStaffEvaluation, getProjectsAwaitingStaffReview, updateProject, deleteProject, listAllProjects } from '../controllers/projectController.js';
...
r.post('/assign', requireAuth, requireRole(['staff', 'admin']), assignProjectToStudent); // Assigner un projet à un étudiant
...
r.get('/awaiting-staff-review', requireAuth, requireRole(['staff', 'admin']), getProjectsAwaitingStaffReview); // Projets en attente de révision staff
...
r.post('/:id/approve', requireAuth, requireRole(['staff','admin']), approveProject);
r.post('/:id/reject', requireAuth, requireRole(['staff','admin']), rejectProject);

// Nouvelle route pour l'évaluation finale par le personnel
r.post('/:id/final-evaluate', requireAuth, requireRole(['staff', 'admin']), submitFinalStaffEvaluation);
